...
def parse_arguments():
    ...
    # Optional Arguments
    parser.add_argument("-v", "--verbose", action="count", default=0, help="""Use -v to display [INFO].
                                                                Use -vv to display [INFO] and [DEBUG].""")
    parser.add_argument("--no-discovery", action="store_true", help="""Turn of automatic adding of hosts
                                                                    as they connect to the server. They
                                                                    must be added manually or using
                                                                    the --whitelist command""")
    parser.add_argument("--threaded", action="store_true", help="""Thread the handlers to 
                                                                handle a large amount of clients""")
    parser.add_argument("--blacklist", nargs=1, help="Don't interact with these IPs")
    parser.add_argument("--whitelist",
                        nargs=1, type=argparse.FileType('r'),
                        help="""Only interact with these IPs. 
                        One IP per line of text file
                        Takes precedence over blacklist.""")
